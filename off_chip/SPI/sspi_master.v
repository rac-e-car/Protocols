module spi_master #(parameter DIV = 50,
        ...
        always @(*) begin
                ...
                case(current_state)
                    ...
                    TRANSFER: begin
                        // BUG FIX: was DATA_WIDTH-1; count reaches DATA_WIDTH after 8 samples
                        if(count == DATA_WIDTH)
                            next_state = DONE;
                        end
            ...
            if(!rst_n) begin
                ...
                case(current_state)
                    ...
                    LOAD: begin
                        ...
                        rx_shift_register <= 0;
                        count             <= 0;
                        // BUG FIX: pre-drive MSB so it is stable before first rising edge
                    ...
                    TRANSFER: begin
                        ...
                        // Shift out on falling edge (AFTER the first rising edge sample)
                        if (falling_edge) begin
                            // BUG FIX: shift first, THEN put next bit on MOSI
                            tx_shift_register <= { tx_shift_register[DATA_WIDTH-2:0], 1'b0 };
                            mosi              <= tx_shift_register[DATA_WIDTH-2]; // next bit
                        end
